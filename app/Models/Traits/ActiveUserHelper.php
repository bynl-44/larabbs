...
//        否则运行匿名函数中的代码来取出活跃用户数据，返回的同时，做了缓存。
...
//        数据按照得分排序
        $users = Arr::sort($this->users, function ($users) {
            return $users['score'];
        });
...
//        根据话题数量计算得分
    ...
    {
//        从回复数据表中取出限定时间范围（$pass_days）内，有发表过数据的用户。
//        并且同时取出用户在此段时间内发表回复的数量
